...
void _s_out(u8 err, char *fmt, ...) {
    ...
    if (err) {
        snprintf(msg_fmt, sizeof(msg_fmt), CRED "[!] TIRDAD: " CNORM "%s\n", fmt);
    } else {
        snprintf(msg_fmt, sizeof(msg_fmt), CGREEN "[-] TIRDAD: " CNORM "%s\n", fmt);
    }
...
u32 secure_tcp_seq_hooked(__be32 saddr, __be32 daddr, __be16 sport, __be16 dport) {
    u32 hash;
...
u32 secure_tcpv6_seq_hooked(const __be32 *saddr, const __be32 *daddr, __be16 sport, __be16 dport) {
    const struct {
    ...
    hash = siphash(&combined, offsetofend(typeof(combined), dport), get_secret());
    return hash;
}
...
#define HANDLER(t, l, ret, ...) \
    t l##h_hk(struct kprobe *kp, struct pt_regs *r __VA_OPT__(,) __VA_ARGS__) { \
        ret; \
    }
...
int hook_init(void) {
    ...
    */
    ...
    if (wait_for_random_bytes()) {
        _s_out(1, "FATAL: Can't get random bytes from the kernel.");
        return -1;
    }
    ...
    */
    ...
    if (install_hook_on(&seqv4) || install_hook_on(&seqv6)) {
